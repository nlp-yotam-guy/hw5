...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            wx = tf.get_variable('W_x', shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            wh = tf.get_variable('W_h', shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable('b', shape=(self.state_size, ), initializer=tf.zeros_initializer())
            new_state = tf.nn.sigmoid(tf.matmul(inputs, wx) + tf.matmul(state, wh) + b)
            ### END YOUR CODE ###
