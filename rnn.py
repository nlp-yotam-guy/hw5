...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) <= max_length:
            padded_sent = [word for word in sentence]
            padded_labels = [label for label in labels]
            padded_mask = [True] * len(sentence)
            k = max_length - len(sentence)
            for i in range(k):
                padded_sent.append(zero_vector)
                padded_labels += [zero_label]
                padded_mask += [False]
        else:
            padded_sent = sentence[:max_length]
            padded_labels = labels[:max_length]
            padded_mask = [True] * max_length
        ret.append((padded_sent, padded_labels, padded_mask))
        ### END YOUR CODE ###
    return ret
